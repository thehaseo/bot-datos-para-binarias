...
def run(tiempo_de_ejecucion_minutos, primera_divisa, segunda_divisa, numero_noticias, horas_noticias):
    print("comenzando")
    datos_operaciones = pd.DataFrame(columns=("precio", "DI+"))
    ...
    while time.time() <= timeout:
            ...
            datos_operaciones["precio"] += datos_5s['c'].iloc[-1]
            ...
            pd.DataFrame.to_csv(datos_operaciones, "datos_operaciones.csv")
            live_data.clear()
            rango_precios.clear()
...
if __name__ == "__main__":
    ...
    while time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())) != f'2020-{mes}-{dia} {hora}:{minuto}:00':
        pass
    run(tiempo, primera_divisa, segunda_divisa, numero_noticias, (noticia1, noticia2, noticia3))
